...
Przetestowane zostaną metody LIME, SHAP, PFI i Grad-CAM, a ich użycie będzie polegało na wytłumaczeniu wyników uzyskanych przez modele Random Forest, XGBoost, sieć neuronową (dane tabelaryczne), VGG-16, ResNet50 oraz Xception (zdjęcia). W tym celu powstał plan eksperymentów dla każdego ze zbiorów. Jest on następujący:
\begin{enumerate}
    \item przeanalizować zbiór danych, w tym ocenić liczbę wartości brakujących, znaleźć kolumny jakościowe;
    \item przetworzyć dane, na co składa się m.in. uzupełnienie braków czy kodowanie zmiennych jakościowych;
    \item zapisać przetworzone zestawy w celu ułatwienia ich wykorzystania w przyszłości;
    ...
    \end{enumerate}
    \item powtórzyć punkty 5. i 6. dla pozostałych modeli.
\end{enumerate}
W przypadku danych zdjęciowych dodatkowo pomiędzy podzieleniem zbioru a nauką pierwszego modelu dochodzi do wybrania badanej próbki, którą ów algorytm przed zostaniem poddanym analizie klasyfikuje.

\subsection{Diabetes Prediction}
Zbiór ,,Diabetes Prediction'' jest problemem klasyfikacji binarnej polegającej na stwierdzeniu, czy dana osoba choruje na cukrzycę. Składa się z 21 zmiennych objaśniających, a~kolumna ,,Diabetes\_binary'', oznaczająca diagnozę, może przyjmować wartości: ,,0'' - brak przypadłości i ,,1'' - diabetyzm.

Jak wskazuje Tabela \ref{tab:metrics_diabetes} \textbf{Random Forest} osiągnął dokładność na poziomie 72\%. Ze~względu na równy rozkład próbek przynależących do poszczególnych klas decyzyjnych, średnie arytmetyczne oraz ważone tej miary były jednakowe. Z~500 przypadków niebędących diabetykami model dobrze rozpoznał 352, co skutkuje specyficznością równą 70\%. Mając do dyspozycji same próbki osób chorujących na cukrzycę model poprawnie zaklasyfikował 367 przypadków. Precyzja osiągnęła poziom 71\%, czułość 73\% a F1-score 72\%. Pole powierzchni pod krzywą ROC wyliczone zostało w przybliżeniu na 0,79. 

W przypadku \textbf{XGBoost'a}, prawie wszystkie przedstawione miary wyniosły 70\%. Jedynie ROC AUC i czułość różniły się, gdzie w pierwszym przypadku model osiągnął 0,77, a drugim 0,69. Rozkłady prawidłowych i błędnych predykcji są bardzo zbliżone. Jako klasa ,,0'' zostało zdiagnozowanych 5 próbek więcej niż do jej alternatywy. 

Wybrana za pomocą przeszukiwania określonej przestrzeni hiperparametrów \textbf{sieć neuronowa} składa się z warstw ukrytych w postaci 64-64 neuronów, używa funkcji aktywacji tanh, mechanizm \textit{Early Stopping}'u oraz optymalizator Adam. Osiągnięta przez nią dokładność wyniosła 72\%, a ROC AUC 0,8. Model częściej zaliczał próbkę jako diabetyka. Znacznie skuteczniej identyfikował tę klasę - o 21 poprawnych przypadków więcej w porównaniu do wyboru alternatywnego. 
...
Z Rys. \ref{fig:shap_absolutemeanplot_rf_diabetes} można wyczytać, że za najważniejszą cechę predykcyjną metoda \textbf{SHAP} uznała ,,GenHlth'', zmieniającą średnio przewidywanie o ok. 9\%. Zaraz po niej w rankingu istotności znajduje się ,,HighBP'' z miarą bardzo zbliżoną do pierwszej zmiennej. Na trzecim miejscu znalazło się ,,BMI'' wpływające na prawdopodobieństwo diagnozy w przybliżeniu o 7\%. Tutaj również zauważamy, iż aż 12 predyktorów miało znikome znaczenie. Ich suma przedstawiona na dole wykresu wyniosła raptem 11\%, co średnio każdej z poszczególnych informacji przypisuje mniej niż 1\% istotności. Ponownie do owych przesłanek zaliczamy kolumny: ,,Fruits'', ,,Veggies'', ,,Smoker'' czy ,,Stroke''.
...
Jak pokazuje wykres znajdujący się na Rys. \ref{fig:pfi_rf_diabetes} metoda \textbf{PFI} wskazała wszystkie cechy za istotne w przewidywaniu. Z Tabeli \ref{tab:pfi_rf_diabetes} możemy wyczytać, że najważniejszą z nich okazała się być ,,GenHlth'', średnio zmieniająca dokładność o 14,6\% z odchyleniem standardowym 0,005. Zmienną o bardzo zbliżonym wyniku jest ,,BMI'', której permutacja zmniejsza miarę o 14,3\%, przy odchyleniu standardowym 0,004. Na trzecim miejscu plasuje się ,,HighBP'' z~wynikiem od 11,2\% do 11,8\%. Najmniej informacji wprowadzały ,,NoDocbcCost'', ,,CholCheck'' oraz ,,AnyHealthcare''. Wartości owych predyktorów oscylowały w granicach ok. 0,4\% - 0,3\%.
...
Jak podaje wykres na Rys. \ref{fig:pfi_xgb_diabetes}, według metody \textbf{PFI}, permutacja którejkolwiek z cech powoduje spadek w dokładności modelu. Największy ma miejsce przy zmianie ,,BMI'' i~wynosi średnio 14,9\% z odchyleniem standardowym rzędu 0,004. Kolejnym predyktorem o dużym znaczeniu jest ,,Age'' z miarą 12,8\% i granicą 0,004. Zaraz po nim znajduje się ,,GenHlth'' z wynikiem od 12,3\% do 12,9\%. Jak czytamy w Tabeli \ref{tab:pfi_xgb_diabetes} trzy ostatnie zmienne nie wpływają na dokładność o więcej niż 1\%, a są to: ,,NoDocbcCost'', ,,Stroke'' oraz ,,AnyHealthcare''.
...
Jak podaje Tabela \ref{tab:pfi_dnn_diabetes}, z 21 cech \textbf{PFI} wskazało tylko 6, których permutacja zawsze miała negatywny wpływ na dokładność. Największe znaczenie wykazało ,,GenHlth'' zmniejszające wspomnianą miarę o 5,5\% z odchyleniem standardowym 0,006. Następnie w owym rankingu znajduje się ,,BMI'' z wynikiem 4,1\% i odchyleniem standardowym rzędu 0,004. Na ostatnim miejscu podium mieści się ,,Age'', a zaraz po nim ,,HighBP'' osiągające odpowiednio 2,3\% i 2,1\%, przy odchyleniu standardowym 0,003. Wpływ dwóch ostatnich zmiennych był niezwykle mały i nie przekraczał 1\%. Na Rys. \ref{fig:pfi_dnn_diabetes} widzimy, że wśród kolumn pominiętych znajdują się, m.in.: ,,Smoker'', ,,Sex'', ,,Fruits'' oraz ,,Veggies''.
...
\begin{xltabular}{\textwidth}{|c|c|c|}
    ...
    \label{tab:3_top_diabetes}\\ \hline
...
Spoglądając na Tabelę \ref{tab:3_top_diabetes} zauważamy, że pojawiły się dwie cechy wśród 3 najistotniejszych występujące w 8 na 9 przypadków. Pierwszą z nich było ,,GenHlth'', które nie wystąpiło tylko w tłumaczeniu LIME w modelu XGBoost, gdzie znalazło się na 4 miejscu. Na~równi z nią widnieje ,,BMI'', którego zabrakło w wyjaśnieniu LIME dla lasu losowego. Najbardziej nieoczekiwaną zmienną w tym zestawieniu jest ,,Income'' wskazane przez LIME modelu XGBoost. Zdawałoby się, że w zestawieniu jest parę innych, bardziej logicznych do uznania predyktorów, takich jak ,,Age'' czy ,,HvyAlcoholConsump'' pojawiające się w innych wytłumaczeniach. 
...
,,UCI Heart Disease Data'' to zestaw danych zawierający informacje pozwalające na klasyfikację wieloklasową - zdiagnozowanie stadium choroby serca. Po przetworzeniu pozostało 11 kolumn objaśniających. Zmienną docelową jest w nim ,,num'' przyjmująca wartości: ,,0'', ,,1'', ,,2'', ,,3'' i ,,4''.  
\subsubsection{Random Forest}
\begin{table}[H]
 \centering
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\linewidth]{rysunki/heart/macierz_pomylek_xgb_heart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{rysunki/heart/lime_barchar_xgb_heart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\linewidth]{rysunki/heart/lime_absolutemeanplot_xgb_heart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.70\linewidth]{rysunki/heart/pfi_xgb_heart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{rysunki/heart/lime_barchar_dnn_heart.png}
...
Na Rys. \ref{fig:lime_absolutemeanplot_dnn_heart}, tak samo jak na poprzednim wykresie, najdłuższy okazuje się być ten słupek należący do ,,oldpeak'', tym razem z wartością ogólną wynoszącą ponad 0,065. Następnie w rankingu znajduje się ,,exang'', a później ,,slope''. Na samym dole umieszczone zostało ,,sex'', które ma wagę 0. Nad nim widnieje ,,trestbps'', a trochę wyżej mieści się ,,cp''.   
...
Według wykresu metody \textbf{SHAP}, zamieszczonego na Rys. \ref{fig:shap_barchar_dnn_heart}, dla wariantu ,,0'' najważniejszą przesłanką okazuje się być ,,cp'' z wartością 0,1, następnie ,,oldpeak'' z 0,08 i~,,exang'' z 0,07. W przypadku ,,1'' najistotniejsze jest ,,exang'', którego miara wynosi 0,06. Dla ,,2'' doszło do ciekawej sytuacji, albowiem aż 3 cechy posiadają jednakowy wynik rzędu ok. 0,03, a są to: ,,age'', ,,dataset'' oraz ,,cp''. Wartości ,,oldpeak'' najmocniej przemawiają w klasach ,,3'' i ,,4''. Prawie w każdym przypadku (oprócz ,,4'') najmniej ważną przesłankę wnosiło ,,trestbps''.    

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{rysunki/heart/shap_barchar_dnn_heart.png}
...
Wszystkie cechy zostały uznane przez \textbf{PFI} jako istotne, jednakże uzyskane przez nie wyniki są szczególnie niskie. Jak wskazuje Tabela \ref{tab:heart_disease_dnn_pfi} najważniejszym parametrem okazał się być ,,oldpeak'' ze spadkiem w dokładności na poziomie jedynie 5,3\% i odchyleniem standardowym 0,011. Na dalszych pozycjach znajduje się ,,dataset'' z wartością 4,5\%, a zaraz po nim ,,exang'' z 4,3\%. Za najmniej istotną przesłankę wskazano ,,trestbps'' z wynikiem wynoszącym 1,1\% i odchyleniem standardowym rzędu 0,004.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.70\linewidth]{rysunki/heart/pfi_dnn_heart.png}
...
Obrazy pochodzące ze zbioru ,,Brain MRI Images for Brain Tumor Detection'' przedstawiają rezonanse magnetyczne mózgów mogące posłużyć do przewidywania, czy dany pacjent posiada zmiany nowotworowe. Zdjęcia w każdym z podzbiorów zostały podzielone na 2 kategorie - ,,no'' oznaczające brak guza oraz ,,yes'' będące jego występowaniem.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\linewidth]{rysunki/brain/gradcam_vgg16_1_brain.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{rysunki/brain/lime_resNet50_1_brain.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\linewidth]{rysunki/brain/gradcam_resNet50_1_brain.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{rysunki/brain/lime_xception_1_brain.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\linewidth]{rysunki/brain/gradcam_xception_1_brain.png}
...
,,Cats and Dogs Classification Dataset'' to zbiór zawierający zdjęci psów i kotów. Zadanie polegało na sklasyfikowaniu, jakie zwierzę znajduje się na obrazku. Fotografie w każdym z podzbiorów zostały przeniesione do odpowiednich plików, gdzie do ,,Cat'' trafiły mruczki, a ,,Dog'' - pieski. 

Z Tabeli \ref{tab:metrics_catsdogs} możemy odczytać, że model \textbf{VGG-16} osiągnął ROC AUC na poziomie 0,99. Precyzja, czułość, specyficzność i F1-score wyniosły odpowiednio 97\%, 96\%, 97\% i~97\%. Dokładność modelu oceniono na 97\%. Średnie arytmetyczne i ważone ze względu na taki sam rozkład próbek względem klas decyzyjnych były jednakowe. Model poprawnie rozpoznał na 1940 z 2000 obrazach kota, a na 1929 z 2000 psa. Tak niezwykle wysoka umiejętność klasyfikacji przełożyła się na fakt, że VGG-16 badanej próbce należącej do klasy ,,0'' przypisał wartość 0,03, a do ,,1'' - 1. Świadczy to o jego dużej pewności, co do wskazanych predykcji, które sa poprawne.

ROC AUC modelu \textbf{ResNet50} było perfekcyjne i osiągnęło 1,0. Precyzja, czułość, specyficzność i F1-score wyniosły 98\%. Taki sam wynik uzyskała także dokładność. Model poprawnie rozpoznał 1953 przypadki z 2000 przedstawiające kota, a 1969 z 2000 obrazów psa. Próbka przynależąca do klasy ,,0'' została zaklasyfikowana z 5\% prawdopodobieństwem do klasy przeciwnej, czyli model był niemal zupełnie pewny jej odmiennej diagnozy. Drugą ResNet50 ocenił na 99\% jako przedstawiającą reprezentanta klasy ,,1''.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{rysunki/cats&dogs/lime_vgg16_0_cats&dogs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\linewidth]{rysunki/cats&dogs/gradcam_vgg16_0_cats&dogs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\linewidth]{rysunki/cats&dogs/gradcam_vgg16_1_cats&dogs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{rysunki/cats&dogs/lime_resNet50_0_cats&dogs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{rysunki/cats&dogs/lime_resNet50_1_cats&dogs.png}
...
Obserwując zamieszcozne na Rys. \ref{fig:gradcam_resNet50_0_cats&dogs} i \ref{fig:gradcam_resNet50_1_cats&dogs} wyjaśnienia \textbf{Grad-CAM} dostrzegamy, że metoda wskazała w obydwu przypadkach dwa duże fragmenty zdjęć. Epicentrum kolorystyczne znalazło się na szyi kota oraz tylnej części torsu psa. Bardziej trafnym oznaczeniem wydaje się być to, osiągnięte przy fotografii psa, gdzie trochę mniej istotnym w danej metryce kolorem oznaczono resztę tułowia stworzenia z kawałkiem jego głowy. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{rysunki/cats&dogs/gradcam_resNet50_0_cats&dogs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{rysunki/cats&dogs/gradcam_resNet50_1_cats&dogs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{rysunki/cats&dogs/lime_xception_0_cats&dogs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{rysunki/cats&dogs/lime_xception_1_cats&dogs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{rysunki/cats&dogs/gradcam_xception_0_cats&dogs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{rysunki/cats&dogs/gradcam_xception_1_cats&dogs.png}
